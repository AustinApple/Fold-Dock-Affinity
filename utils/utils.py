...
def read_UniProt_complex_dataframe(path):
...
def get_optimizer_and_scheduler(args, model, scheduler_mode='min'):
    ...
    if args.scheduler == 'plateau':
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode=scheduler_mode, factor=0.7,
                                                               patience=args.scheduler_patience, min_lr=args.lr / 1000, verbose=True)
    else:
        print('No scheduler')
        scheduler = None
...
def get_model(args, device, t_to_sigma, no_parallel=False, confidence_mode=False):
    ...
    if confidence_mode:
        print('####### args of rmsd_classification_cutoff in get model', args.rmsd_classification_cutoff)
