...
class PDBBind(Dataset):
    ...
    def preprocessing(self):
        ...
        if self.esm_embeddings_path is not None:
            ...
            for key, embedding in id_to_embeddings.items():
                ...
                if key_name in set(df_lig_rec['uniprot_id']):
                    chain_embeddings_dictlist[key_name].append(embedding)
            lm_embeddings_chains_all = []
            for name in list(df_lig_rec['uniprot_id']):
        ...
        if self.num_workers > 1:
            ...
            complex_names = list(df_lig_rec.index)
            ligands_smi = list(df_lig_rec['ligand_smi'])
            recs = list(df_lig_rec['uniprot_id'])
            affinities = list(df_lig_rec['kd'])
            lm_embeddings_chains = lm_embeddings_chains_all
            complex_graphs, rdkit_ligands = [], []
            with tqdm(total=len(df_lig_rec), desc='loading complexes') as pbar:
                for t in map(self.get_complex, zip(complex_names, lm_embeddings_chains, ligands_smi, recs, affinities)):
                    complex_graphs.extend(t[0])
                    rdkit_ligands.extend(t[1])
                    pbar.update()
    ...
    def get_complex(self, par):
        name, lm_embedding_chains, ligand_smi, rec, affinity = par
    
        try:
            rec_model = parse_pdb_from_path(os.path.join(self.pdbbind_dir, f'{rec}.pdb'))
        ...
        ligs =[]
        mol = MolFromSmiles(ligand_smi)  # check if it is a smiles or a path
        if mol is not None:
            mol = AddHs(mol)
            generate_conformer(mol)
