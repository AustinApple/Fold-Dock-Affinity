...
from affinity.dataset_davis import AffinityDataset
...
parser.add_argument('--alpha', type=float, default=0.5, help='the weight of classification')
...
def train_epoch(model, loader, optimizer, affinity_prediction, alpha):
    model.train()
    meter = AverageMeter(['MSE'])
    ...
    for data in tqdm(loader, total=len(loader)):
        ...
        optimizer.zero_grad()
        try:
            predict_regression = model(data)
            print(f'#### predict_regression {predict_regression}')
            if affinity_prediction:
        
                labels_regression = torch.cat([graph.affinity for graph in data]).to(device) if isinstance(data, list) else data.affinity.to(device)
                
                # (labels - min(labels)) / (max(labels) - min(labels)) this is the normalization based on DAVIS dataset
                labels_regression = (labels_regression - 5) / 5.54
    
                
                MSE_loss = F.mse_loss(predict_regression, labels_regression)
...
def test_epoch(model, loader, affinity_prediction, alpha):
    model.eval()
    meter = AverageMeter(['MSE'], unpooled_metrics=True) if affinity_prediction else AverageMeter(['confidence_loss', 'accuracy', 'ROC AUC'], unpooled_metrics=True)
    all_labels = []
    all_affinities = []
    for data in tqdm(loader, total=len(loader)):
        try:
            ...
            if affinity_prediction:
                ...
                # transform to e^9 to reproduce original result
                predict_regression = (predict_regression * 5.54) + 5

                
                MSE_loss = F.mse_loss(predict_regression, labels_regression)
                
                
                #accuracy = accuracy_score(labels_classification.detach().cpu().numpy(), predict_classification.detach().cpu().numpy())
                #accuracy = 1
                #roc_auc = roc_auc_score(labels_classification.detach().cpu().numpy(), predict_classification.detach().cpu().numpy())
...
def train(args, model, optimizer, scheduler, train_loader, val_loader, run_dir):
    ...
    for epoch in range(args.n_epochs):
        logs = {}
        train_metrics = train_epoch(model, train_loader, optimizer, args.affinity_prediction, args.alpha)
        print("Epoch {}: Training loss {:.4f}".format(epoch, train_metrics['MSE']))

        val_metrics = test_epoch(model, val_loader, args.affinity_prediction, args.alpha)
        if args.affinity_prediction:
            print("Epoch {}: Validation total loss {:.4f}".format(epoch, val_metrics['MSE']))
            # print("Epoch {}: Validation MSE loss {:.4f}".format(epoch, val_metrics['MSE']))
            #print("Epoch {}: Validation accuracy {:.4f}".format(epoch, val_metrics['accuracy']))
            #print("Epoch {}: Validation roc auc {:.4f}".format(epoch, val_metrics['ROC_AUC']))
        else:
        ...
        if args.wandb:
            ...
            logs.update({'mean_rmsd' if args.affinity_prediction else 'fraction_positives': baseline_metric,
...
def construct_loader_confidence(args, device):
    common_args = {'cache_path': args.cache_path, 'original_model_dir': args.original_model_dir,
                   'inference_steps': args.inference_steps, 'samples_per_complex': args.samples_per_complex,
                   'limit_complexes': args.limit_complexes, 'balance': args.balance, 'rmsd_classification_cutoff': args.rmsd_classification_cutoff, 'all_atoms': args.all_atoms,
                   ...
                   'batch_size': args.batch_size, 'confidence_model_dir': args.confidence_model_dir, 'confidence_ckpt': args.confidence_ckpt, 
                   'no_random': args.no_random, 'ode': args.ode, 'no_final_step_noise': args.no_final_step_noise }
    # loader_class = DataListLoader if torch.cuda.is_available() else DataLoader
    loader_class = DataLoader
    exception_flag = False
    try:
        train_dataset = AffinityDataset(split="train", args=args, **common_args, device=device)
        train_loader = loader_class(dataset=train_dataset, batch_size=args.batch_size, shuffle=False)
    ...
    val_dataset = AffinityDataset(split="val", args=args, **common_args, device=device)
...
if __name__ == '__main__':
    ...
    # construct loader
    train_loader, val_loader = construct_loader_confidence(args, device=device)
    model = get_model(score_model_args if args.transfer_weights else args, device, t_to_sigma=None, confidence_mode=True, no_parallel=True)
